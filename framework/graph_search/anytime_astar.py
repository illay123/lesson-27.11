    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                current_aster = AStar(heuristic_function_type=self.heuristic_function_type,
                                      heuristic_weight=((low_heuristic_weight + high_heuristic_weight) / 2),
                                      max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration)
                result = current_aster.solve_problem(problem=problem)
                ...
                if not result.is_solution_found:
                    low_heuristic_weight = (low_heuristic_weight + high_heuristic_weight) / 2
                    continue
                high_heuristic_weight = (low_heuristic_weight + high_heuristic_weight) / 2
                if best_solution.solution_g_cost > result.solution_g_cost:
                    best_heuristic_weight = high_heuristic_weight
